// events.js — 事件/告警活动面板（导出: loadActivity, switchTab, updateAlarmTabBadge, ack）
// 依赖: util.js 的 esc/reportError/clearError

// 告警确认（当前告警 tab 的确认按钮）
async function ack(id) { await fetch('/api/alarms/' + id + '/ack', { method: 'POST' }); loadActivity(); }

// ---------- 活动面板（事件/命令/告警，仅本页激活时轮询；全数组指纹 diff：空闲轮询 0 DOM 变更） ----------
let actFp = { ev: '', cmd: '', hist: '', alarms: '' };
async function loadActivity() {
  ...
  try {
    const [rEv, rCmd, rHist, rAlarms] = await Promise.all([
      apiGet('/api/events?tail=50'),
      apiGet('/api/commands?tail=50'),
      apiGet('/api/alarm-history?tail=50'),
      apiGet('/api/alarms'),
    ]);
    if (!rEv.fresh || !rCmd.fresh || !rAlarms.fresh) return;   // 过期响应丢弃
    const ev = rEv.data, cmd = rCmd.data, hist = rHist.data, alarms = rAlarms.data;
    Object.values(panels).forEach(clearError);

    const evFp = ev.map(e => `${e.time}|${e.ceid}|${e.description}`).join('¶');
    if (evFp !== actFp.ev) {
      actFp.ev = evFp;
    ...
    const cmdFp = cmd.history.map(c => `${c.time}|${c.source}|${c.station}|${c.cmd}|${c.ok}|${c.elapsedMs}`).join('¶') + '#' + cmd.pending.length;
    if (cmdFp !== actFp.cmd) {
      actFp.cmd = cmdFp;
    ...
    const histFp = hist.map(h => `${h.time}|${h.action}|${h.unitId}|${h.alarmId}|${h.text}`).join('¶');
    if (histFp !== actFp.hist) {
      actFp.hist = histFp;
    ...
    const alarmsFp = alarms.map(a => `${a.unitId}|${a.alarmId}|${a.alarmText}|${a.acked}`).join('¶');
    if (alarmsFp !== actFp.alarms) {
      actFp.alarms = alarmsFp;
