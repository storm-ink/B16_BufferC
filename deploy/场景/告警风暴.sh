#!/bin/bash
# 场景4：告警风暴 —— 16 站同码快速注入 → SC 级 102 去重只 1 次；清除 → 101
# 断言：mcs 收到 102（仅 1 次）与 101
set -e
...
# 生成 16 站 × 3 轮注入命令（同告警码 9 → SC 级 102 去重；t5 起）
...
echo "== 先起 PLC 仿真（t5 风暴注入 → mcs 验证 102 去重与 101 清除）"
( printf "$PLC_SCRIPT" \
...
OK=1
grep -q "CEID 102" "$WORK/mcs.log" && echo "    ✓ mcs 收到 CEID 102（SC 级告警）" || { echo "    ✗ mcs 未收到 102"; OK=0; }
grep -q "CEID 101" "$WORK/mcs.log" && echo "    ✓ mcs 收到 CEID 101（SC 级清除）" || { echo "    ✗ mcs 未收到 101"; OK=0; }
